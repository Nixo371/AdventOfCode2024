...
class Simulator {
	...
	public:
		...
		std::set<std::pair<int, int>> get_unique_positions() {
			return (this->unique_positions);
		}
};

bool is_in_bounds(std::vector<std::string> map, int row, int col) {
	return (row >= 0 && row < map.size() && col >= 0 && col < map.at(0).size()); 
}

bool simulate_loop(std::vector<std::string> map, int guard_row, int guard_col) {
	std::vector<std::pair<int, int>> dirs({{-1, 0}, {0, 1}, {1, 0}, {0, -1}}); // North, East, South, West
	std::set<std::vector<int>> states = std::set<std::vector<int>>();
	int dir = 0;
	while (is_in_bounds(map, guard_row, guard_col)) {
		if (states.count({guard_row, guard_col, dir}) == 1) {
			return (true);
		}
		states.insert({guard_row, guard_col, dir});
		int dr = dirs[dir].first;
		int dc = dirs[dir].second;

		if (!is_in_bounds(map, guard_row + dr, guard_col + dc)) {
			break;
		}

		if (map.at(guard_row + dr).at(guard_col + dc) == '#') { // Turn right
			dir = (dir + 1) % 4;
			continue;
		}
...
int main(int argc, char *argv[]) {
	...
	std::set<std::pair<int, int>> unique_positions = sim.get_unique_positions();
	
	int total_iterations = unique_positions.size();
	int iteration = 0;
	int loop_positions = 0;
	for (std::pair<int, int> pos : unique_positions) {
		lines.at(pos.first).at(pos.second) = '#';
		loop_positions += simulate_loop(lines, guard_row, guard_col);
		lines.at(pos.first).at(pos.second) = '.';
		std::cout << "\r" << ++iteration << "/" << total_iterations;
	}
	std::cout << "Total Loop Positions: " << loop_positions << std::endl;
}
