...
std::set<std::pair<int, int>> calculate_antinodes(const std::vector<std::vector<char>> map, const std::vector<std::pair<int, int>> antenna) {
	...
	for (size_t first = 0; first < antenna.size() - 1; first++) {
		for (size_t second = first + 1; second < antenna.size(); second++) {
			...
			std::pair<int, int> antinode_1 = {pos_2.first + diff.first, pos_2.second + diff.second};
			std::pair<int, int> antinode_2 = {pos_1.first - diff.first, pos_1.second - diff.second};

			if (is_in(antinode_1.first, antinode_1.second, rows, cols)) {
				antinodes.insert(antinode_1);
			}

			if (is_in(antinode_2.first, antinode_2.second, rows, cols)) {
